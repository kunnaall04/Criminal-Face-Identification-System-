...
def basicPageSetup(pageNo):
    ...
    back_img_path = os.path.join(BASE_DIR, "back.png")
    back_img = tk.PhotoImage(file=back_img_path)
    ...
    right_frame = tk.LabelFrame(content, text=" ed Criminals", bg="#202d42", font="Arial 20 bold", bd=4,
...
def selectMultiImage(opt_menu, menu_var):
    ...
    if(len(path_list) < 5):
        ...
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        prev_img_path = os.path.join(BASE_DIR, "previous.png")
        next_img_path = os.path.join(BASE_DIR, "next.png")
        back_img = tk.PhotoImage(file=prev_img_path)
        next_img = tk.PhotoImage(file=next_img_path)
...
def register(entries, required, menu_var):
    ...
    # Setting Directory
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    path = os.path.join(BASE_DIR, "face_samples", "temp_criminal")
    ...
    if(len(no_face) > 0):
        ...
        rowId = insertData(entry_data, host='localhost', 
                   user='root',  
                   password='Kunal@2468', 
                   database='cfis')
        ...
        if(rowId > 0):
            ...
            base_dir = os.path.dirname(os.path.abspath(__file__))
            shutil.move(path, os.path.join(base_dir, 'face_samples', entry_data["Name"]))
            ...
            cv2.imwrite(os.path.join(profile_dir, "criminal %d.png"%rowId), img_list[profile_img_num])
...
def showCriminalProfile(name):
    ...
    (id, crim_data) = retrieveData(name, host='localhost',
                                user='root',
                                password='Kunal@2468',
                                database='cfis')

    base_dir = os.path.dirname(os.path.abspath(__file__))
    path = os.path.join(base_dir, "profile_pics", "criminal %d.png"%id)
    ...
    for i, item in enumerate(crim_data.items()):
        ...
        if raw_val is None or raw_val == "":
            ...
            try:
                ...
                display_val = "---"
        tk.Label(info_frame, text=display_val.capitalize() if isinstance(display_val, str) else display_val, fg="white", font="Arial 15", bg="#202d42").grid(row=i, column=2, sticky='w')
...
try:
    # Open image with PIL and convert to RGBA mode to support transparency
...
if logo_image:
    # Place the image label on top-left corner
